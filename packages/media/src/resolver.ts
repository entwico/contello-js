...
export type MediaResolverOptions = {
  ...
  /** `<source>` format cascade, most-preferred first — the first format with variants wins. default: AVIF, then WebP */
...
export class MediaResolver<HasDefault extends boolean = false> {
  ...
  private resolveImageSource(source: ImageInputArg | null | undefined, options?: ImageSourceOptions): ImageSource {
    ...
    // formats is a priority cascade — the first format with usable variants provides the
    // single <source>; later formats only apply when the earlier ones have no variants
    for (const format of formats) {
      ...
      const srcset = toSrcset(variants);

      if (srcset) {
        sources.push({ type: format, srcset });
        break;
      }
    }
    ...
    if (sources.length === 0 && mainTypeVariants.length > 1) {
      const srcset = toSrcset(mainTypeVariants);
  ...
  private assetToImageDef(asset: MediaAssetInput): ImageDef {
    ...
    for (const file of optimized) {
      const variant = this.rasterVariant(file);
    ...
    const previewVariant = this.rasterVariant(preview);
  ...
  // a raster variant without positive dimensions was never actually generated by the
  // processor — it must not participate in resolution (it would produce an invalid
  // `0w` srcset descriptor and win/lose size-based picks arbitrarily)
  private rasterVariant(file: DeepReadonly<MediaFile> | null | undefined): ImageDefVariant | null {
...
// browser discards one). keep the first after the ascending sort. non-positive widths are
// skipped — a `0w` descriptor is invalid and makes the browser drop the candidate.
function toSrcset(variants: ImageDefVariant[]): string {
  const byWidth = new Map<number, ImageDefVariant>();
  const sorted = variants.filter((v) => v.width > 0).toSorted((a, b) => a.width - b.width);
